...
import com.ttknp.springmailsenderandjasper.dto.OrderItemDTO;
...
public class OrderItemService implements OrderItemDTO {
    ...
    public HashMap<String, byte[]> getOrderItemsHasMapReport(String fileType) {
        ...
        if (fileType != null) {
            try {
                ...
                byte[] fileReport = ordersHistoryListJasperReportInBytesFromRootPathForExpose(fileType);
    ...
    public HashMap<String, byte[]> getOrderItemsHasMapReport(String fileType, String datetime) { // it's same getOrderItemsHasMapReport(fileType) but just have parameters
        ...
        if (fileType != null) {
            try {
                ...
                byte[] fileReport = ordersHistoryListJasperReportInBytesFromRootPathForExpose(fileType, datetime);
                map.put(fileName, fileReport);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    @Override
    public HashMap<String, byte[]> getOrderItemsHasMapReport() {
        HashMap<String, byte[]> map = new HashMap<>(); // key is filename & value is file
        String fileName = "order_items.pdf";
        //  byte[] fileReport = ordersHistoryListJasperReportInBytesRootPath(fileType);
        byte[] fileReport = null;
        try {
            fileReport = ordersHistoryListJasperReportInBytes();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // log.debug("file report : {}", fileReport); // [102, 10, 54, 49, 57, 55, 51, 49, 48, 10, 37, 37, 69, 79, 70, 10 ,...]
    ...
    private byte[] ordersHistoryListJasperReportInBytesFromRootPathForExpose(String fileType) throws Exception {
    ...
    private byte[] ordersHistoryListJasperReportInBytesFromRootPathForExpose(String fileType, String datetime) throws Exception {
        String resourceTemplateClassPath = "classpath:report/orders_history_list_basic_template.jrxml";
    ...
    private byte[] ordersHistoryListJasperReportInBytes() throws Exception {
        String resourceTemplateClassPath = "classpath:report/orders_history_list_basic_template.jrxml";
        List<OrderItem> orderItemsDataSource = getOrderItemsForJasperReportLimit10();
        Map<String, Object> parameters = new HashMap<>(); // ** can be null
        parameters.put("title", "Orders History");
        parameters.put("logoUrl", "http://www.thitikorn-nupan.com/app/ecommerce/logo.png");
        parameters.put("totalPrice", 100001.15);
        ...
        JasperReport jasperReport = JasperCompileManager.compileReport(path);
        JasperPrint jasperPrint = JasperFillManager.fillReport(jasperReport, parameters, beanCollectionDataSource);
        // Export to PDF in memory using ByteArrayOutputStream
